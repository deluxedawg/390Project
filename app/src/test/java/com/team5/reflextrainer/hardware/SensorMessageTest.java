...
public class SensorMessageTest {
    ...
    public void parsesValidResult() {
        // reaction time 300ms = 0x012C -> lo=0x2C, hi=0x01
    ...
    public void rejectsBadChecksum() {
        byte[] msg = frame(SensorMessage.RESP_RESULT, SensorMessage.OUTCOME_CORRECT, 0x2C, 0x01);
        msg[5] = (byte) (msg[5] ^ 0xFF);   // corrupt the checksum
        assertNull(SensorMessage.parse(msg));   // must be rejected
    }
    ...
    public void buildsChallengeWithCorrectBytes() {
        // timeout 1000ms = 0x03E8 -> lo=0xE8, hi=0x03
        ...
        assertEquals((byte) 0xE8, out[3]);   // lo
        assertEquals((byte) 0x03, out[4]);   // hi
    }

    @Test
    public void challengeRoundTripsThroughChecksum() {
        // a challenge we build should itself pass parse()'s checksum logic
        byte[] out = SensorMessage.buildChallenge(
                SensorMessage.MSG_START_CHALLENGE, SensorMessage.TARGET_SHAKE_IMU, 500);
        byte start = out[0], b1 = out[1], b2 = out[2], b3 = out[3], b4 = out[4];
        byte expected = (byte) (start ^ b1 ^ b2 ^ b3 ^ b4);
